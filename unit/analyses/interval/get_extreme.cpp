...
{
  GIVEN("A selection of constant_exprts in a std::vector")
  {
    ...
    for(int i = -100; i <= 100; i++)
    {
      ...
      WHEN("Two are selected and min found [20, -20]")
      {
        ...
        THEN("Min (-20) and max (20) are verified")
        {
          ...
          REQUIRE(min.get(ID_value) == "-20");
          REQUIRE(max.get(ID_value) == "20");
        }
      }
    ...
    WHEN("One is selected [-100]")
    {
      ...
      {
        REQUIRE(min.get(ID_value) == "-100");
        REQUIRE(max.get(ID_value) == "-100");
      }
    }
