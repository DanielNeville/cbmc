...
void path_symext::assign(path_symex_statet &state, const exprt &lhs,
		const exprt &rhs) {
	...
	exprt ssa_lhs = state.read_no_propagate(dereference_exprt(lhs_address));
...
void path_symext::symex_malloc(path_symex_statet &state, const exprt &lhs,
...

	{
		...
		if (tmp_size.id() == ID_mult && tmp_size.operands().size() == 2
			...
			object_type = array_typet(c_sizeof_type_rec(tmp_size.op0()),
					tmp_size.op1());
		} else {
			...
			if (tmp_type.is_not_nil()) {
				...
				mp_integer elem_size = pointer_offset_size(tmp_type,
						state.var_map.ns);
					...
					if (alloc_size == elem_size)
						...
						if (elements * elem_size == alloc_size)
							object_type = array_typet(tmp_type,
									from_integer(elements, tmp_size.type()));
					}
				}
			}
		}
		...
		if (object_type.id() == ID_array
			...
			size_symbol.base_name = "dynamic_object_size"
					+ i2string(dynamic_count);
			...
			assign(state, size_symbol.symbol_expr(), size);
	...
	value_symbol.base_name = "dynamic_object"
			+ i2string(state.var_map.dynamic_count);
...
void path_symext::assign_rec(path_symex_statet &state, exprt::operandst &guard,
		const exprt &ssa_lhs, const exprt &rhs) {
	...
	if (ssa_lhs.id() == ID_symbol) {
		...
		if (ssa_rhs.is_nil()) {
			/*
			 *  Propagate/instantiate taint. By default, non-det implies
			 *	untaint.
			 */
			path_symex_statet::var_statet &var_state = state.get_var_state(
					var_info);
			...
			if (state.taint_engine.enabled) {
				...
				var_state.taint =
						(state.is_enforced_taint_json()) ?
								state.get_enforced_taint() :
								taint_enginet::get_top_elem();
			}
		} else {
			// consistency check
			if (!base_type_eq(ssa_rhs.type(), new_lhs.type(),
					state.var_map.ns)) {
#ifdef DEBUG
			...
			if (state.taint_engine.enabled) {
				std::cout << "Setting taint (not-nil) prop from RHS: "
						<< ssa_rhs.pretty() << " .\n";
				...
				std::cout << "calculated taint: "
						<< state.taint_engine.get_taint_name(taint) << "\n";
		...
		const typet &compound_type = state.var_map.ns.follow(struct_op.type());
		...
		if (compound_type.id() == ID_struct) {
			...
			member_name.set(ID_component_name,
			...
			byte_extract_exprt new_lhs(byte_update_id(), struct_op, offset,
					ssa_rhs.type());
		...
		const byte_extract_exprt &byte_extract_expr = to_byte_extract_expr(
				ssa_lhs);
		...
		const struct_typet &struct_type = to_struct_type(
				state.var_map.ns.follow(ssa_lhs.type()));
		...
		const array_typet &array_type = to_array_type(ssa_lhs_type);
		...
		const vector_typet &vector_type = to_vector_type(
				state.var_map.ns.follow(ssa_lhs.type()));
		...
		for (unsigned i = 0; i < operands.size(); i++) {
			...
			|| ssa_lhs.id() == "NULL-object" || ssa_lhs.id() == "zero_string"
...
void path_symext::function_call_rec(path_symex_statet &state,
		const code_function_callt &call, const exprt &function,
	...
	if (function.id() == ID_symbol) {
...

		{
			...
			function_call_rec(further_states.back(), call, if_expr.false_case(),
					further_states);
		}
		...
		// ignore
		function_call_rec(state, call, to_typecast_expr(function).op(),
				further_states);
	} else
...
void path_symext::return_from_function(path_symex_statet &state) {
	...
	if (thread.call_stack.empty()) {
		...
		for (path_symex_statet::var_state_mapt::const_iterator it =
				thread.call_stack.back().saved_local_vars.begin();
				it != thread.call_stack.back().saved_local_vars.end(); it++)
...
void path_symext::set_return_value(path_symex_statet &state, const exprt &v) {
	path_symex_statet::threadt &thread =
			state.threads[state.get_current_thread()];
...
void path_symext::do_goto(path_symex_statet &state, bool taken) {
	state.record_step();
...
void path_symex_set_taint_via_symbols(path_symex_statet &state,
	...
	const taint_enginet &taint_engine = state.taint_engine;
