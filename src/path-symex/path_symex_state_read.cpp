...
exprt path_symex_statet::array_theory(const exprt &src, bool propagate) {
	...
	if (src.id() == ID_index) {
		...
		if (!index_tmp2.is_constant()) {
			const array_typet &array_type = to_array_type(
					index_expr.array().type());
			const typet &subtype = array_type.subtype();
			...
			if (array_type.size().is_constant()) {
				...
				for (std::size_t i = 0; i < size_int; ++i) {
					...
					exprt new_src = index_exprt(index_expr.array(), index,
							subtype);
...
exprt path_symex_statet::instantiate_rec(const exprt &src, bool propagate) {
#ifdef DEBUG
	...
	if (is_symbol_member_index(src)) {
		exprt tmp_symbol_member_index = read_symbol_member_index(src,
				propagate);
	...
	if (src.id() == ID_address_of) {
		...
		const typet &compound_type = var_map.ns.follow(
				to_member_expr(src).struct_op().type());
		...
		assert(src.type().id() == ID_code || src.get_bool(ID_C_SSA_symbol));
...
exprt path_symex_statet::read_symbol_member_index(const exprt &src,
		bool propagate) {
	const typet &src_type = var_map.ns.follow(src.type());
	...
	if (final.id() == ID_struct || final.id() == ID_array
			|| final.id() == ID_vector) {
	...
	while (current.id() != ID_symbol) {
		...
		if (current.id() == ID_member) {
			...
			const typet &compound_type = var_map.ns.follow(
					member_expr.struct_op().type());

			if (compound_type.id() == ID_struct) {
				// go into next iteration
				next = member_expr.struct_op();
				suffix = "." + id2string(member_expr.get_component_name())
						+ suffix;
			} else
	...
	irep_idt identifier = to_symbol_expr(current).get_identifier();

	var_mapt::var_infot &var_info = var_map(identifier, suffix, src.type());
...
exprt path_symex_statet::dereference_rec(const exprt &src, bool propagate) {
...
exprt path_symex_statet::instantiate_rec_address(const exprt &src,
		bool propagate) {
#ifdef DEBUG
	...
	if (src.id() == ID_symbol) {
		...
		if_expr.true_case() = instantiate_rec_address(if_expr.true_case(),
				propagate);
		if_expr.false_case() = instantiate_rec_address(if_expr.false_case(),
				propagate);
